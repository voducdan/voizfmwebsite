...
export default function AudioPlay({ audioFromApi }) {
  ...
  useEffect(() => {
    if (audio) {
      const { id, slug } = audio;
      const newUrl = getSharedLink(slug, SharedType.Audio);
      setId(id);
      setUrl(newUrl);
    }
  }, [audio]);
  ...
  return (
    <Box
      sx={{
        ...
        margin: `${isSm ? HEADER_HEIGHT_MB : HEADER_HEIGHT} auto ${
          isSm ? "auto" : "100px"
        } auto`,
      ...
      >
        ...
        >
          <img
            ...
            alt="playlist avt"
            src={
              audio?.avatar?.original_url ||
              audio?.playlist?.avatar?.original_url
            }
          />
        </Box>
        <Box
        ...
        >
          ...
          <TableContainer
            ...
            component={Paper}
          >
            <Table aria-label="playlist info tbl">
              <TableBody>
                <TableRow
