...
export default function PlatlistDetail({ playlistFromAPI }) {
  ...
  const [audio, setAudio] = useState(
    typeof Audio !== "undefined" ? new Audio(audioTrailerUrl) : undefined
  );
  ...
  const [afterRateContent, setAfterRateContent] = useState(
    "Cảm ơn đánh giá của bạn. Bạn có thể thay đổi điểm đánh giá  bất cứ lúc nào."
  );
  ...
  useEffect(() => {
    async function fetchPlaylist() {
      ...
      const playlistTrailer =
        playlistFromAPI.playlist_trailers.length > 0
          ? playlistFromAPI.playlist_trailers[0]["file_url"]
          : "";
  ...
  const createPlaylistInfo = () => {
    if (Object.keys(playlist).length > 0) {
      const playlistInfo = [
        {
          ...
          value: (
            <Link href={`/authors/${playlist?.authors[0]?.id}`}>
              <Box
                sx={{
                  cursor: "pointer",
                }}
        ...
        },
        {
          ...
          value: (
            <Typography
              sx={{ ...TEXT_STYLE.content2, color: COLORS.VZ_Text_content }}
            >
              {convertSecondsToReadableString(playlist?.total_duration)}
            </Typography>
          ),
        },
        {
          ...
          value: (
            <Link href={`/channels/${playlist?.channel?.id}`}>
              <Box
                sx={{
                  cursor: "pointer",
                }}
        ...
        },
        {
          ...
          value: (
            ...
            >
              <InfoValue
                value={playlist?.voicers.map((i) => i.name).join(", ")}
              />
            </Box>
          ),
        },
        {
          ...
          value: (
            <Box
              sx={{ ...flexStyle("flex-start", "center"), columnGap: "2px" }}
            >
              <Typography
                sx={{ ...TEXT_STYLE.content2, color: COLORS.VZ_Text_content }}
              >
                {formatRating(playlist?.playlist_counter?.content_avg)}
              </Typography>
              <svg
                width="14"
                height="13"
              ...
              <Typography
                sx={{ ...TEXT_STYLE.content2, color: COLORS.VZ_Text_content }}
              >
                ({playlist?.playlist_counter?.ratings_count})
              </Typography>
            </Box>
          ),
        },
      ...
      if (
        playlist?.promotion === "coin" ||
        (playlist?.promotion === "vip" &&
          playlist?.category?.code !== "podcast")
      ) {
        const sellPrice = {
              ...
              {playlist?.sale_coin_price < playlist?.coin_price && (
                ...
                >
                  {playlist?.sale_coin_price < playlist?.coin_price && (
                    ...
                    >
                      {FormatPrice(playlist?.coin_price * 100)}đ
                    </Typography>
                  )}
                  <Typography
                    sx={{ ...TEXT_STYLE.content2, color: COLORS.white }}
                  >
                    {FormatPrice(playlist?.sale_coin_price * 100)}đ
                  </Typography>
                </Box>
              )}
              {playlist?.sale_coin_price === playlist?.coin_price && (
    ...
    >
      ...
      >
        <img
          ...
          alt="cover img alt"
          src={playlist?.avatar?.original_url}
        ></img>
      </Box>
      <Box
        sx={{
              ...
              >
                ...
                >
                  ...
                  <Button
                    ...
                    startIcon={
                      playlist?.is_bookmark ? <CheckIcon /> : <AddIcon />
                    }
                  >
                    {playlist?.is_bookmark ? "Hủy đánh dấu" : "Đánh dấu"}
                  </Button>
                  <Snackbar
                    open={openSnackbar}
        ...
        >
          ...
          {!!audioTrailerUrl && (
            <Button
              ...
              startIcon={
                paused ? (
                  <VolumeUpIcon sx={{ color: COLORS.white }} />
                ) : (
                  <VolumeMuteIcon sx={{ color: COLORS.white }} />
                )
              }
              onClick={onPlayClick}
            >
            ...
            {!isSm && (
              ...
              >
                {recommendedPlaylist.map((item, idx) => (
                  <Link href={"/play/[id]"} as={`/play/${item?.id}`} key={idx}>
                    <Box
                      sx={{
                ...
                {playlistAudios.map((value, idx) => {
                  return (
                    <ListItem
                      key={value.id}
                      onClick={() => {
                        handlePlayOneAudio(value?.id, idx);
                      }}
                      sx={{
                        paddingLeft: 0,
                        paddingRight: 0,
      ...
      >
        {["vip", "coin"].includes(playlist?.promotion) &&
          !playlist?.is_purchased &&
          playlist?.category?.code !== "podcast" && (
            <Tooltip
              open={addToCartError}
              title={
                <div style={{ whiteSpace: "pre-line", color: COLORS.error }}>
