...
export default function AudioLike() {
  ...
  const [openUnLikeAudioErrorSnackbar, setOpenUnLikeAudioErrorSnackbar] =
    useState(false);
  ...
  return (
    <Box sx={{ width: "100%" }}>
          ...
          <Button
            onClick={() => {
              handleConfirmDeleteModalOpen(
                "Bạn có chắc chắn muốn xóa những audio này không?"
              );
            }}
            sx={{
              ...(isSm ? TEXT_STYLE.title2 : TEXT_STYLE.title1),
      ...
      <Snackbar
        ...
        onClose={() => {
          setOpenUnLikeAudioErrorSnackbar(false);
        }}
      >
        <Alert
          onClose={() => {
